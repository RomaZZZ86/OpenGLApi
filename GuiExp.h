...
public:
    ...
    GLTextureNano() = default;
    GLTextureNano(const std::string& textureName)
        : mTextureName(textureName), mTextureId(0) {}

    GLTextureNano(const std::string& textureName, GLint textureId)
        : mTextureName(textureName), mTextureId(textureId) {}

    GLTextureNano(const GLTextureNano& other) = delete;
    GLTextureNano(GLTextureNano&& other) noexcept
        : mTextureName(std::move(other.mTextureName)),
        mTextureId(other.mTextureId) {
        other.mTextureId = 0;
    }
    GLTextureNano& operator=(const GLTextureNano& other) = delete;
    GLTextureNano& operator=(GLTextureNano&& other) noexcept {
        mTextureName = std::move(other.mTextureName);
        std::swap(mTextureId, other.mTextureId);
        return *this;
    }
    ~GLTextureNano() noexcept {
        if (mTextureId)
...
private:
    ...
    using imagesDataType = vector<pair<GLTextureNano, GLTextureNano::handleType>>;
